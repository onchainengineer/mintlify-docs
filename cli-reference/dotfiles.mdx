...
```console
wirtual dotfiles [flags] <git_repo_url>
```
...
```console
  - Check out and install a dotfiles repository without prompts:
     ...
     $ wirtual dotfiles --yes git@github.com:example/dotfiles.git
```
...
| Environment | <code>$WIRTUAL_SYMLINK_DIR</code> |
...
| ----------- | ------------------------------------- |
| Type        | <code>string</code>                   |
| Environment | <code>$WIRTUAL_DOTFILES_REPO_DIR</code> |
